...
// A country is only useful to display if it has a flag image
function hasFlag(country) {
  return Boolean(country.flag?.url_png || country.flag?.url_svg);
}
...
export async function getAllCountries() {
  ...
  // Skip countries without a flag image — they break the UI
  return allCountries.filter(hasFlag);
}
...
export async function getCountryByCode(code) {
  ...
  if (objects.length === 0) {
    ...
    if (exact) {
      if (!hasFlag(exact)) throw new Error("Country has no flag data");
      return exact;
    }
  }
  ...
  if (!hasFlag(country)) throw new Error("Country has no flag data");
  return country;
}
